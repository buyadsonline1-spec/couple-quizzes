...
// ======================================================
// ПОДТВЕРЖДЕНИЕ ОПЛАТЫ (pre_checkout_query)
// ======================================================
// Telegram шлёт этот запрос ПОСЛЕ того, как пользователь нажал
// "Подтвердить" в инвойсе, и ждёт answerPreCheckoutQuery в течение
// 10 секунд — иначе сам отменяет платёж с ошибкой у пользователя
// (ровно то, что видели: бесконечная загрузка → ошибка). Этого
// обработчика раньше не было вообще, поэтому оплата Stars не
// проходила НИ для одного плана (Premium, буст, суперлайк, вещи
// питомца) — не только для новых pet_item.
...
// Вещи для питомца за Stars (корона/медаль/пуховик/комната «Космос») —
// цену проверяет сама create-stars-invoice (PET_ITEM_STARS), здесь
// только выдача владения через grant_pair_pet_item, которая НЕ
// списывает solo_points и не проверяет уровень/достижения (это уже
// оплачено реальными деньгами, а не заработано). Список item_id,
// которые grant_pair_pet_item реально примет, зашит в саму RPC — так
