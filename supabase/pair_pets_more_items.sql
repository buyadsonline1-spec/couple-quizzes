-- 6 новых бесплатных вещей (по 2 на шапки/аксессуары/куртки),
-- выдаются просто за уровень питомца, без цены вообще:
--   hat_wizard (ур.2), hat_bandana (ур.5)
--   acc_necklace (ур.2), acc_headphones (ур.5)
--   jacket_raincoat (ур.2), jacket_vest (ур.5)
-- Применять в Supabase → SQL Editor, целиком, одним запуском. Требует
-- уже применённых pair_pets.sql, pair_pets_shop.sql,
-- pair_pets_unlocks.sql, pair_pets_jackets.sql.

-- buy_pair_pet_item — та же RPC из pair_pets_jackets.sql, плюс каталог
-- 6 новых вещей. У них price=0 (а не null/stars-only) — это НАСТОЯЩАЯ
-- бесплатная вещь: покупка через эту функцию спишет 0 очков и просто
-- зарегистрирует владение, как только разблокирована по уровню.
